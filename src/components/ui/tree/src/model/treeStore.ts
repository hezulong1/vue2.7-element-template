...
import { isUndefinedOrNull, isObject } from '@/utils/types';
...
export default class TreeStore {
  ...
  append(data: TreeNodeData, parentData: TreeNodeData | TreeKey | Node): void {
    const parentNode = !isUndefinedOrNull(parentData)
  ...
  _initDefaultCheckedNode(node: Node): void {
    ...
    if (!isUndefinedOrNull(node.key) && defaultCheckedKeys.includes(node.key)) {
      node.setChecked(true, !this.checkStrictly);
    }
  }
  ...
  registerNode(node: Node): void {
    ...
    if (!key) {
      ...
      if (!isUndefinedOrNull(nodeKey)) this.nodesMap[nodeKey] = node;
    }
  }
