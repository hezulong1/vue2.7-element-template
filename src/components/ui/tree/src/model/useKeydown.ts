...
export function useKeydown({ el$ }: UseKeydownOption, store: Ref<TreeStore>) {
  ...
  const handleKeydown = (ev: KeyboardEvent): void => {
    ...
    if (!currentItem.className.includes('el-tree__node')) return;
    ...
    const hasInput = currentItem.querySelector<HTMLInputElement>('[type="checkbox"]');
    if (
      [EVENT_CODE.enter, EVENT_CODE.numpadEnter, EVENT_CODE.space].includes(code) &&
      hasInput
    ) {
      ev.preventDefault();
      hasInput.click();
  ...
  function initTabIndex() {
    ...
    const treeItems = Array.from(el$.value.querySelectorAll('.is-focusable[role=treeitem]'));
